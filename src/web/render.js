...
function getChartMeta(chartId) {
  if (chartId === 'hp') return { icon: '❤️', colorClass: 'chart-hp' };
...
function renderCompactPick(item) {
  return `
    <span class="compact-pick${item.boss ? ' boss' : item.elite ? ' elite' : ''}">
    ...
    return `
      <div class="page page-upload-only">
        ...
        <section class="upload-card${state.dragActive ? ' drag-active' : ''}" data-dropzone="true">
          ...
          <button class="upload-drop-box" type="button" data-action="choose-file">
        ...
        ${renderPanel(report.allCardPicksTitle, report.allCardPicks.length ? `
          <div class="pick-groups">
            <div class="pick-legend">
              <span class="compact-pick-kind normal">${escapeHtml(state.locale === 'zh' ? '普通' : 'Normal')}</span>
              <span class="compact-pick-kind elite">${escapeHtml(state.locale === 'zh' ? '精英' : 'Elite')}</span>
              <span class="compact-pick-kind boss">${escapeHtml(state.locale === 'zh' ? 'Boss' : 'Boss')}</span>
            </div>
            ${report.allCardPicks.map(group => `
              <article class="pick-group">
                <strong>${escapeHtml(group.actName)}</strong>
                <div class="compact-picks">
                  ${group.picks.map(renderCompactPick).join('')}
                </div>
              </article>
            `).join('')}
          </div>
          ...
          <div class="floor-detail">
            ...
            <article class="floor-card">
              ...
              <div class="floor-stats">
                ${selectedFloor.hp ? `<span>❤️ ${escapeHtml(selectedFloor.hp)}</span>` : ''}
              ...
              ${selectedFloor.rewardChoices.picked || selectedFloor.rewardChoices.skipped.length ? `
                <div class="floor-section">
                  <div class="floor-section-title">🃏 ${escapeHtml(report.text.choices)}</div>
                  <div class="choice-pills">
                    ${selectedFloor.rewardChoices.picked ? renderEntity(selectedFloor.rewardChoices.picked, true) : ''}
                    ${selectedFloor.rewardChoices.skipped.map(entry => renderEntity(entry, false)).join('')}
                  </div>
                </div>
              ` : ''}
              ...
              ` : ''}
              ${selectedFloor.relicRewards.length || selectedFloor.potionRewards.length ? `
                <div class="floor-section">
                  <div class="floor-section-title">🎁 ${escapeHtml(report.text.rewards)}</div>
                  <div class="choice-pills">
                    ${selectedFloor.relicRewards.map(entry => renderEntity(entry, true)).join('')}
                    ${selectedFloor.potionRewards.map(entry => renderEntity(entry, true)).join('')}
                  </div>
                </div>
              ` : ''}
            </article>
          </div>
        ` : '<div class="empty">-</div>', 'panel-half')}
